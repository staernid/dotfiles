...
def check_compressibility(fin, total_bytes, block_size):
    """
    Quickly estimate the compression ratio of the file.
    Samples a small number of blocks to decide if full compression is worth it.
    """
    ...
    if total_block == 0:
        ...
        # Sample a maximum of 100 blocks distributed across the file
        sample_count = min(total_block, 200)
        step = total_block // sample_count
        sample_blocks = [i * step for i in range(sample_count)]

    compressed_size = 0
    original_size = 0

    # Save original position
    original_pos = fin.tell()
    ...
    for block_idx in sample_blocks:
        ...
        # Use fast compression for the estimate
        try:
            z_data = lz4_compress(data, level=1)
            compressed_size += len(z_data)
            original_size += len(data)
        except:
            # If a block fails, just skip it for the estimate
            continue

    # Restore original position
    fin.seek(original_pos)
...
def usage():
    print("Usage: ziso [-c level] [-m] [-t percent] [-f percent] [-d] [-h] infile [outfile]")
    print("       ziso [-c level] [-m] [-t percent] [-f percent] [-d] [-h] infile1 infile2 ...")
    ...
    print("  -f percent Fast check compressibility threshold (1-100, default 80), abort if not met")
    print("  -a align Padding alignment 0=small/slow 6=fast/large")
    print("  -p pad Padding byte")
    print("  -d Delete source file after successful conversion (or prompt at end if multiple files)")
    print("  -h this help")
...
def compress_zso(fname_in, fname_out, level, bsize):
    ...
    if FAST_CHECK_THRESHOLD < 100:
        print("Performing fast compressibility check...", end=' ', flush=True)
        ratio = check_compressibility(fin, total_bytes, bsize)
        print("Estimated ratio: %.2f%%" % ratio)
        if ratio > FAST_CHECK_THRESHOLD:
            print(fname_in)
            print("File is not compressible enough (estimated ratio %.2f%% > threshold %d%%). Aborting.\n" % (ratio, FAST_CHECK_THRESHOLD))
            fin.close()
            return False
...
def parse_args():
    global MP, COMPRESS_THREHOLD, FAST_CHECK_THRESHOLD, DEFAULT_PADDING, DEFAULT_ALIGN, PROMPT_DELETE
    ...
    try:
        optlist, args = gnu_getopt(sys.argv, "c:b:mt:f:a:p:hd")
    ...
    for o, a in optlist:
        if o == '-c':
            ...
            FAST_CHECK_THRESHOLD = min(int(a), 100)
        elif o == '-a':
            DEFAULT_ALIGN = int(a)
        elif o == '-p':
    ...
    if level is None:
        level = 2 # Default to level 2 as seen in ziso wrapper

    files = args[1:]
    if not files:
        print("You have to specify at least one input filename")
        usage()
        sys.exit(-1)

    file_pairs = []
    
    # Check if we have (infile, outfile) or just list of infiles
    if len(files) == 2:
        file_in = files[0]
        file_out = files[1]
        
        # Heuristic: if first is .iso and second is .zso (or vice versa for level 0)
        # then it's (infile, outfile)
        is_iso_zso = file_in.lower().endswith('.iso') and file_out.lower().endswith('.zso')
        is_zso_iso = file_in.lower().endswith('.zso') and file_out.lower().endswith('.iso')
        
        if (level > 0 and is_iso_zso) or (level == 0 and is_zso_iso):
            file_pairs.append((file_in, file_out))
        else:
            # Treat as two separate infiles
            for f in files:
                ext = '.zso' if level > 0 else '.iso'
                out = os.path.splitext(f)[0] + ext
...
def main():
    ...
    level, bsize, file_pairs = parse_args()

    success_files = []
    
    for fname_in, fname_out in file_pairs:
        if not os.path.exists(fname_in):
            print(f"Error: {fname_in} not found, skipping.")
            continue
            
        success = False
        if level == 0:
            success = decompress_zso(fname_in, fname_out)
        else:
            success = compress_zso(fname_in, fname_out, level, bsize)
        
        if success:
            success_files.append(fname_in)

    if success_files and PROMPT_DELETE:
        print("\nConversion finished.")
